...
def post_list(request):
    posts = Post.publushed.all()[:5]
    most_recent = Post.publushed.all().order_by('-timestamp', '-updated')[:3]
...
def lazy_load_posts(request):
    ...
    # Build a html post list with the pagenated posts
...
def post_by_category(request, category_slug):
    ...
    return render(request, 'posts/post_by_category.html', context)
...
def post_by_tag(request, tag_slug):
    ...
    return render(request, 'posts/post_by_tag.html', context)
...
def favourite_post(request):
    ...
    if post.favourite.filter(id=request.user.id).exists():
        ...
        post.favourite.aadd(request.user)
        is_favourite = True
    ...
    if request.is_ajax():
        html = render_to_string('posts/favorite_section.html', context, request=request)
